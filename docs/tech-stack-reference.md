...
- 只复制未完成任务：开放任务全复制（父-子结构保持）；已关闭任务一律不带入、留在原周；异常数据下已关闭节点的未完成后代会提升到父级继续带入。
...
- 树防护：不能移动到自身/子树；往已关闭节点挂任务（新建/移动/拖拽）允许，后端 `reopen_closed_chain` 自动重开该节点及祖先链，保持「已关闭节点下无开放子任务」不变量；前端 `computeDrop` 与后端 domain 双重校验。
